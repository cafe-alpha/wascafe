...
//#define SPI_MAX10_SND_RSP_READY() HAL_GPIO_WritePin(GPIOA, SPI_SYNC_Pin, GPIO_PIN_SET  )
  ...
  while (1)
  {
    ...
    if(spi_tick_delta > 5)
    {
        ...
        char data_log[32+1];
        memcpy(data_log, trans_hdr->data, sizeof(data_log)-1); data_log[sizeof(data_log)-1] = '\0';
unsigned char char2pchar(unsigned char c);
        for(int i=0; i<(sizeof(data_log)-1); i++)
        {
            data_log[i] = char2pchar(data_log[i]);
        }
