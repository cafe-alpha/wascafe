...
sdc_ret_t sdc_fat_init(void)
{
    ...
    sd->init_status = ret;
    sdc_logout(" [sdc_fat_init]sdc_init ret=%d SDC_OK[%d]", ret, SDC_OK);
    if(ret != SDC_OK)
    {
        return ret;
    }
    ...
    /* Init FAT32 library. */
    sdc_logout(" [sdc_fat_init]fl_init STT ...", 0);
    fl_init();
    sdc_logout(" [sdc_fat_init]fl_init END", 0);

    sdc_logout(" [sdc_fat_init]fl_attach_media STT ...", 0);
    ret = fl_attach_media(fat_media_read, fat_media_write);
    sdc_logout(" [sdc_fat_init]fl_attach_media END", 0);
    ...
    sdc_logout(" [sdc_fat_init]sdc_init terminate OK !", 0);
    sd->init_ok = 1;
    return SDC_OK;
}
    ...
    if (fl_opendir(path, &dirstat))
    {
        ...
        while (fl_readdir(&dirstat, &dirent) == 0)
        {
            ...
            && (file_index < (list_offset+list_count)))
            {
                ...
                /* Verify that file/folder name fits within limitations. */
                dirent.filename[FATFS_MAX_LONG_FILENAME-1] = '\0';
                int name_length = (int)strlen(dirent.filename);
                if(name_length >= SC_FILENAME_MAXLEN)
                {
                    list_ptr[list_index].flags |= FILEENTRY_FLAG_NAMETOOLONG;
                }

                strncpy(list_ptr[list_index].filename, dirent.filename, SC_FILENAME_MAXLEN);

                /* If file name too long, copy the last four characters
                 *  of the filename to output buffer.
                 * Purpose is to keep file extension so that it can be
                 * correctly filtered, and consequently file access error
                 * message can be displayed.
                 */
                if((!dirent.is_dir)
                && (list_ptr[list_index].flags | FILEENTRY_FLAG_NAMETOOLONG))
                {
                    memcpy(list_ptr[list_index].filename + SC_FILENAME_MAXLEN-5, dirent.filename + name_length - 4, 4);
                }
