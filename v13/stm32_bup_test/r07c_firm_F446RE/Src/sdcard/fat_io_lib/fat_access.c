        ...
        if (fatfs_sector_reader(fs, Cluster, x++, 0)) // If sector read was successfull
        {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
            {
                ...
                if (fatfs_entry_lfn_text(directoryEntry) )
                    fatfs_lfn_cache_entry(&fg->lfn, fs->currentsector.sector+recordoffset);

                // If Invalid record found delete any long file name information collated
                else if (fatfs_entry_lfn_invalid(directoryEntry) )
                    fatfs_lfn_cache_init(&fg->lfn, 0);

                // Normal SFN Entry and Long text exists
                else if (fatfs_entry_lfn_exists(&fg->lfn, directoryEntry) )
                {
                    long_filename = fatfs_lfn_cache_get(&fg->lfn);
...
int fatfs_update_timestamps(struct fat_dir_entry *directoryEntry, int create, int modify, int access)
{
    ...
    struct tm dummy_time_info = { 0 };
    time_info = &dummy_time_info;
#endif
