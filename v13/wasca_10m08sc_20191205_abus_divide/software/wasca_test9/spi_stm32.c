...
void spi_get_version(wl_spicomm_version_t* ver)
{
    ...
    hdr->cmn.magic_ca = 0xCA;
    hdr->cmn.magic_fe = 0xFE;
    hdr->cmn.command  = WL_SPICMD_VERSION;
    hdr->cmn.pkt_len  = 0; // TODO
    hdr->cmn.rsp_len  = 0; // TODO
     ...
     */
    memcpy(ver, hdr->params, sizeof(wl_spicomm_version_t));
...
void spi_memread(unsigned long addr, unsigned long len, unsigned char* data)
{
     ...
     */
    if(len > WL_SPI_PARAMS_LEN)
    {
        len = WL_SPI_PARAMS_LEN;
    }
    ...
    hdr->cmn.magic_ca = 0xCA;
    hdr->cmn.magic_fe = 0xFE;
    hdr->cmn.command  = WL_SPICMD_MEMREAD;
    hdr->cmn.pkt_len  = 0; // TODO
    hdr->cmn.rsp_len  = 0; // TODO

    /* Setup memory read parameters. */
    wl_spicomm_memacc_t* params = (wl_spicomm_memacc_t*)hdr->params;
    params->addr = addr;
    params->len  = (unsigned short)len;
...
void spi_memwrite(unsigned long addr, unsigned long len, unsigned char* data)
{
    ...
    hdr->cmn.magic_ca = 0xCA;
    hdr->cmn.magic_fe = 0xFE;
    hdr->cmn.command  = WL_SPICMD_MEMWRITE;
    hdr->cmn.pkt_len  = 0; // TODO
    hdr->cmn.rsp_len  = 0; // TODO

    /* Setup memory write parameters. */
    wl_spicomm_memacc_t* params = (wl_spicomm_memacc_t*)hdr->params;
    unsigned char* wr_data = hdr->params + sizeof(wl_spicomm_memacc_t);
    params->addr = addr;
    params->len  = (unsigned short)len;
    memcpy(wr_data, data, len);
...
//}
    ...
    hdr->cmn.magic_ca = 0xCA;
    hdr->cmn.magic_fe = 0xFE;
    hdr->cmn.pkt_len = params[0];
    hdr->cmn.rsp_len = params[3];
