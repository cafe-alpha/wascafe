...
/* The header file where each GPIO ports are defined.
 *
 * Note : pins with the following labels are bitbanged.
 *  - SD_CS_GPIO_Port   / SD_CS_Pin
 *  - SD_DIN_GPIO_Port  / SD_DIN_Pin
 *  - SD_CLK_GPIO_Port  / SD_CLK_Pin
 *  - SD_DOUT_GPIO_Port / SD_DOUT_Pin
 */
#include <main.h>
...
void sc_usleep(unsigned long micro_cnt)
{
    unsigned long tick_stt = HAL_GetTick();

    while(1)
    {
        if((HAL_GetTick() - tick_stt) >= ((micro_cnt / 1000) + 1))
        {
            break;
        }
    }
...
//int _dly_first_log = 1;
void sdc_small_wait(void)
{
#if 0
//unsigned long ticks_stt = HAL_GetTick();
    // On F446RE, 1K loops ~= 24usec
    const int count = 4;
    for(int i=count; i; i--)
    {
        asm("nop");
    }
//unsigned long ticks_end = HAL_GetTick();
//unsigned long dly_usec = (1000 * (ticks_end - ticks_stt)) / (count/1000);
//if(_dly_first_log)
//{
//    sdc_logout("small_wait: ticks[%u] usec[%u]/Kcnt", ticks_end-ticks_stt, dly_usec);
//}
//_dly_first_log = 0;
#endif
}
...
void sdc_output(void)
{
    ...
    HAL_GPIO_WritePin(SD_CS_GPIO_Port , SD_CS_Pin , (sd->s.csl ? GPIO_PIN_SET : GPIO_PIN_RESET));
    HAL_GPIO_WritePin(SD_DIN_GPIO_Port, SD_DIN_Pin, (sd->s.din ? GPIO_PIN_SET : GPIO_PIN_RESET));
    HAL_GPIO_WritePin(SD_CLK_GPIO_Port, SD_CLK_Pin, (sd->s.clk ? GPIO_PIN_SET : GPIO_PIN_RESET));
    sdc_small_wait();
}
...
void sdc_sendbyte_array(unsigned char* ptr, unsigned short len)
{
    ...
    for(/*nothing*/; len; len--)
    {
        ...
        for(bit=7; bit>=0; bit--)
        {
            /* Set current bit, and then generate a pulse (L -> H) on CLK line. */
            HAL_GPIO_WritePin(SD_DIN_GPIO_Port, SD_DIN_Pin, (dat & (1 << bit) ? GPIO_PIN_SET : GPIO_PIN_RESET));
            sdc_small_wait();
...
void sdc_receivebyte_array(unsigned char* ptr, unsigned short len)
{
    ...
    for(/*nothing*/; len; len--)
    {
        ...
        for(bit=7; bit>=0; bit--)
        {
            /* Generate a pulse (H -> L) on CLK line and then read current bit from DOUT. */
            HAL_GPIO_WritePin(SD_CLK_GPIO_Port, SD_CLK_Pin, GPIO_PIN_SET  );
            sdc_small_wait();
            HAL_GPIO_WritePin(SD_CLK_GPIO_Port, SD_CLK_Pin, GPIO_PIN_RESET);
            sdc_small_wait();

            if(HAL_GPIO_ReadPin(SD_DOUT_GPIO_Port, SD_DOUT_Pin))
            {
                tmp |= (1 << bit);
            }
        }
...
#else
        /* It's a little broken ... but doesn't uses float computation.
         * Anyway, computing user size needs to be re-done from reading the docs.
         */
        tmpl = (c_size * c_size_mult * block_length) / 1048576;
        sd->stats.user_size = tmpl;
#endif
...
unsigned char sdc_sendpacket(unsigned char cmd, unsigned long arg_tmp, unsigned char* buffer, unsigned long blocks_count)
{
    ...
    **/
    ...
    // for(pollcnt=0; pollcnt<256; pollcnt++)
    // {
    //     tmp = 0;
    //     for(i = 0; i < 8; i++)
    //     {
    ...
    // } // Byte loop
    // 
    // /* Verify if SD card is ready to receive command or not. */
    // if(tmp != 0xFF)
    // {
    //     sdc_logout("~sdc::cmd%-2d[ERROR] Command synchronization timeout ! arg=0x%08X(%d)", 
    //         cmd, 
    //         arg, arg);
    //     sd->packet_timeout = 1;
    // 
...
//sdc_logout("### CMD0 8 clocks tmp=0x%02X", tmp);
#endif
    ...
    do
    {
        ...
        tmp = (HAL_GPIO_ReadPin(SD_DOUT_GPIO_Port, SD_DOUT_Pin) == GPIO_PIN_RESET ? 0 : 1);
