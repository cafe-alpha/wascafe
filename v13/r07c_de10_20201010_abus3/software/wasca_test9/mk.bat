...
@REM Copy readme and register map near archive file
copy /Y readme.txt %archive_full_base%_readme.txt
copy /Y ..\wasca_register_map.txt %archive_full_base%_regmap.txt

@REM Archive current register map document, and keep a track of previous versions
set regmap_file=R:\wasca_register_map_%date:~0,4%%date:~5,2%%date:~8,2%.txt
copy ..\wasca_register_map.txt %regmap_file%
..\..\7za a -tzip -mx=9 %pj_workdir%\wasca_register_map.zip %regmap_file%
del /Q %regmap_file%
